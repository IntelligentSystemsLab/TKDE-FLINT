...
class Client(nn.Module):
    def __init__(self,model,id,path,type_client,update_step,lr,device,batch_size):
        ...
        self.label_cnt = np.zeros(shape=10)
    ...
    def moon_local_train(self, global_net, mu, tau):
        ...
        for _ in range(self.update_step):
            for x, target in self.support_loader:
                ...
                # loss1 = self.CE_loss(pro1, target)
                pro2, _ = global_net(x)
                posi = cos(pro1, pro2)
    ...
    def client_test(self, net):
        ...
        local_ave_loss = loss_all / total
        local_ave_acc = acc_all / total

        return local_ave_loss, local_ave_acc
